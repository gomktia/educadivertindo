import os

def fix_pixel_script():
    filepath = "/Users/pantera/Desktop/Projetos/Recetario Ancestral/vendas.html"
    
    with open(filepath, "r", encoding="utf-8") as f:
        content = f.read()
    
    pixel_script = """<!-- Meta Pixel Code -->
<script>
!function(f,b,e,v,n,t,s)
{if(f.fbq)return;n=f.fbq=function(){n.callMethod?
n.callMethod.apply(n,arguments):n.queue.push(arguments)};
if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';
n.queue=[];t=b.createElement(e);t.async=!0;
t.src=v;s=b.getElementsByTagName(e)[0];
s.parentNode.insertBefore(t,s)}(window, document,'script',
'https://connect.facebook.net/en_US/fbevents.js');
fbq('init', '1536230360782560');
fbq('track', 'PageView');
</script>
"""

    if "fbq('init'" not in content:
        if "</head>" in content:
            content = content.replace("</head>", pixel_script + "</head>")
            print("Inserted Pixel Script before </head>.")
        else:
            print("Error: </head> tag not found.")
            # Fallback to before Utmify if we can match it exactly this time
            # But </head> was confirmed by view_file so we trust it.
    else:
        print("Pixel Script already present (unexpected if previous step failed checks).")

    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    
    print("Fix script completed.")

if __name__ == "__main__":
    fix_pixel_script()
